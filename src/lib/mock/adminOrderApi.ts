...
export const adminOrderAPI = {
  ...
  getById: async (id: string): Promise<{ success: boolean; order: OrderDetails }> => {
    await delay(600);
    ...
    if (!order) {
      ...
      const defaultOrder: OrderDetails = {
        ...
        timeline: [
          { status: 'Order Placed', date: `${basicOrder.date} 10:00`, icon: 'Clock', completed: true },
          { status: 'Payment Confirmed', date: `${basicOrder.date} 10:05`, icon: 'CreditCard', completed: true },
          { status: 'Processing', date: null, icon: 'Package', completed: false },
          { status: 'Shipped', date: null, icon: 'Truck', completed: false },
          { status: 'Delivered', date: null, icon: 'CheckCircle2', completed: false },
        ],
      };
  ...
  exportOrders: async (): Promise<{ success: boolean; downloadUrl: string }> => {
    await delay(1000);
