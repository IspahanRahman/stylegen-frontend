...
const mockProducts: Record<string, ProductDetails> = {
  "1": {
    ...
    stock: "25",
    description:
      "Handcrafted Italian leather weekend bag with premium materials.",
    sizes: "One Size",
    images: [
...
export const adminProductCRUDAPI = {
  getById: async (
    id: string,
  ): Promise<{ success: boolean; product: ProductDetails }> => {
    await delay(600);
  ...
  create: async (
    ...
    const newProduct: ProductDetails = {
      ...
      images: images.map((img, i) =>
        img.startsWith("blob:")
          ? `/images/products/uploaded-${Date.now()}-${i}.jpg`
          : img,
      ),
      status: "active",
      featured: false,
  ...
  uploadImages: async (
    ...
    try {
      ...
      const urls = files.map(
        (_, i) => `/images/products/img-${Date.now()}-${i}.jpg`,
      );
      return { success: true, urls };
    }
  },
